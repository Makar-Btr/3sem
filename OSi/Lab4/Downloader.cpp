...
void ProcessFile()
{
    ...
    static std::mt19937 rng(
        static_cast<unsigned>(std::time(nullptr) ^ GetCurrentProcessId()));
...
int main()
{
    ...
    {
        LogMessage(hMutex, L"[PID: " + std::to_wstring(pid) +
                               L"] Cancelled (waiting in queue).");
    }
    else if (waitResult == WAIT_OBJECT_0 + 1)
    {
        ...
        else
        {
            ...
            {
                LogMessage(hMutex, L"[PID: " + std::to_wstring(pid) +
                                       L"] ! Interrupted during process.");
            }
            else
            {
                LogMessage(hMutex,
                           L"[PID: " + std::to_wstring(pid) + L"] + Done.");
                ReleaseSemaphore(hSemaphore, 1, NULL);
            }
        }
    }
